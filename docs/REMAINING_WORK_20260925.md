...
- [x] Review the 104 currently flagged published articles against their supplied sources; preserve evidence and distinguish uncertain checks from proven errors. Public article replacement remains a separate editorial action.
...
- [x] Release checks, independent reviews, CI, merge, preview sync and backend deployment. Provider transaction tests and a full cross-browser matrix remain outside the verified coverage.
...
The reviewed release head `c6507789` includes all eight repair groups above and was merged in PR20 as `d289c680`. Independent TypeScript, general code, and PostgreSQL reviews found issues in upload cancellation, dynamic contents links, uncertain note retries, historical follow-up measurement, and old reporting denominators; those issues were fixed and regression-tested. The two SQL migrations are `20260925150000_news_concurrency.sql` and `20260925160000_offer_journey_measurement.sql`.

Local validation: 2,348 tests across 209 files passed; the production build, production-worker smoke check, TypeScript, ESLint (zero errors; 286 warnings), all edge-function type checks, and formatting passed. GitHub verification run 153 also passed all checks, including 32 isolated database suites. The phone-sized browser check confirmed working resource filters, shareable contents links after filtering, no horizontal overflow, and separate chat/Summit controls after resizing. This is not an assertion that every provider/browser combination was tested.

Backend deployment is complete for `generate-news-article`, `generate-content`, `render-page`, `conversion-events`, `offers-api`, and `offer-stripe-webhook` (shared offer response dependency). Both migrations are applied and their stored source matches the reviewed files. Do not replay them. The focused Lovable deployment cost 0.8 credits. Lovable then generated a schema type update in `9d6d0cdb`; that diff received a separate review and passed TypeScript and targeted ESLint. The updated resource pages and giveaway were verified in preview. No public frontend publication, provider transactions, customer messages, or retry cron is part of this release.
...
An item-by-item private editorial review covers the 104 articles/151 flagged claims: 51 confirmed errors, 31 supported claims, 20 recommendations or hypothetical examples incorrectly treated as factual assertions, and 49 unverified claims. All 91 reviewed working copies are saved in private editor recovery, including 47 complete replacement drafts where misleading premises recur. Thirteen articles have review notes without replacement drafts. Full replacements use corrected titles, descriptions and sources, with stale summaries, FAQs and images removed. Read-back checks confirmed all 91 snapshots match exactly, all 104 published source articles remain unchanged, and the pre-existing working copy was preserved. Source flags have not been cleared automatically. The private review reports and draft bodies are deliberately excluded from GitHub.
