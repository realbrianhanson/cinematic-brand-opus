...
**Unsupported source instruction corrected:** The refresh and resource-generation prompts also labeled a fixed list of tools, including Jasper and Copy.ai, as defunct/outdated. Their current official pages still describe available platforms, so this was not a defensible retirement rule. These prompts now require a current linked source for a retirement claim. Refresh does not treat absence from search results as evidence that a tool has closed; new generation requires source-supported recommendations. Resource and guide generators also label retrieved output as research material, not independently verified information, and generated fallback metadata no longer fabricates a verification claim. [Jasper platform](https://www.jasper.ai/platform), [Copy.ai platform](https://www.copy.ai/platform/powering-gtm-ai).
...
76 focused tests pass across eight files: missing-page server/client behavior and matching admin instructions, resource labels, search comparisons, the freshness endpoint 15 mocked refresh-safety cases, and two generator prompt-policy checks. Scoped lint has zero errors (existing edge-renderer `any` warnings remain), and Deno checking passes for all five changed edge entries. The coordinator's final integration checks cover full application typing and the complete test suite. No production records were changed and nothing was deployed by this subtask. Frontend/server-entry changes arrive in Lovable preview through GitHub; `render-page`, `check-content-freshness`, `refresh-stale-content`, `generate-content` and `generate-pillar` require separate managed-function deployment. No database migration is included.
