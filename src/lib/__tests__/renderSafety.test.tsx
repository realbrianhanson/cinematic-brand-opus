...
import { Asset, createRootRoute, createRouter, RouterContextProvider } from "@tanstack/react-router";
...
describe("server rendering safety", () => {
  it("keeps rich article content and removes executable HTML without a DOM", () => {
    ...
    const html = renderToStaticMarkup(<article dangerouslySetInnerHTML={{ __html: safeHtml(content) }} />);
  ...
  it("cannot break out of the real TanStack JSON-LD script renderer", () => {
    const data = { headline: '</script><script>alert("stored")</script>\u2028\u2029' };
    const script = buildPageHead({ title: "Test", description: "Test", url: "https://example.com", jsonLd: [data] }).scripts[0];
    const router = createRouter({ routeTree: createRootRoute() });
    const html = renderToStaticMarkup(<RouterContextProvider router={router}><Asset tag="script" attrs={{ type: script.type }} children={script.children} /></RouterContextProvider>);
