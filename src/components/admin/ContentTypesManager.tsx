...
const ContentTypesManager = () => {
  ...
  const { data: pageCounts } = useQuery({
    ...
    queryFn: async () => {
      ...
      (data ?? []).forEach((p) => {
        if (p.content_schema_id)
          counts[p.content_schema_id] = (counts[p.content_schema_id] || 0) + 1;
      });
      return counts;
    },
  });
  ...
  const duplicateMutation = useMutation({
    mutationFn: async (schema: Tables<"content_schemas">) => {
  ...
  const bulkCreateMutation = useMutation({
    mutationFn: () =>
      safeMutation(async () => {
        ...
        const toCreate = CONTENT_TYPE_TEMPLATES.filter(
          (t) => !existingSlugs.has(t.slug),
        );
        if (toCreate.length === 0)
          throw new Error("All 6 content type templates already exist.");
        const { error } = await supabase.from("content_schemas").insert(
          toCreate.map((t) => ({
            name: t.name,
            slug: t.slug,
            ...
            schema_definition: t.schema_definition as Json,
            is_active: true,
          })),
        );
        if (error) throw error;
            ...
            <tbody>
              {schemas.map((s) => (
                ...
                >
                  ...
                  >
                    {s.title_template?.length > 40
                      ? s.title_template.slice(0, 40) + "…"
                      : s.title_template}
                  </td>
                  <td
                    className="font-body"
