...
const NichesManager = () => {
  ...
  const removeTag = (type: "subtopics" | "keywords_seed", idx: number) => {
    setForm((p) => ({
      ...
      context: {
        ...p.context,
        [type]: (p.context[type] ?? []).filter((_, i) => i !== idx),
      },
    }));
  };
  ...
  const handleCsvFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 1024 * 1024) {
      toast({
        title: "CSV too large",
        description: "Choose a CSV smaller than 1 MB.",
        variant: "destructive",
      });
      return;
    }
    reader.onerror = () =>
      toast({ title: "CSV read failed", variant: "destructive" });
    reader.onload = (ev) => {
      try {
        if (!niches)
          throw new Error(
            "Wait for your niches to finish loading, then import.",
          );
        const rows = parseNicheCsv(
        ...
        toast({
          ...
          description:
            error instanceof Error ? error.message : "Cannot read CSV.",
          variant: "destructive",
        });
      }
    };
  ...
  return (
    <div>
            ...
            onClick={() => {
              ...
              if (!ids.length) {
                toast({
                  ...
                  description: "All filtered niches are already active.",
                });
                return;
              }
              if (
                !confirm(
                  `Activate ${ids.length} niche${ids.length === 1 ? "" : "s"}?`,
                )
              )
                return;
            ...
            onClick={() => {
              ...
              if (!ids.length) {
                toast({
                  ...
                  description: "All filtered niches are already inactive.",
                });
                return;
              }
              if (
                !confirm(
                  `Deactivate ${ids.length} niche${ids.length === 1 ? "" : "s"}?`,
                )
              )
                return;
            ...
            return (
              ...
              >
                ...
                {ctx.audience && (
                  ...
                  >
                    {ctx.audience.length > 80
                      ? ctx.audience.slice(0, 80) + "…"
                      : ctx.audience}
                  </p>
                )}
                <div
          ...
          >
            ...
            <ModalField label="Parent Niche (optional)">
              <select
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, parent_niche_id: e.target.value }))
                }
              >
            ...
            <ModalField label="Audience" htmlFor="niche-audience">
              <textarea
                ...
                value={form.context.audience ?? ""}
                onChange={(e) =>
                  setForm((p) => ({
                    ...p,
                    context: { ...p.context, audience: e.target.value },
                  }))
                }
            ...
            <ModalField label="Pain Points">
              <textarea
                className="admin-input font-body"
                rows={3}
                placeholder="What are their biggest frustrations?"
                value={form.context.pain_points ?? ""}
                onChange={(e) =>
                  setForm((p) => ({
                    ...p,
                    context: { ...p.context, pain_points: e.target.value },
                  }))
                }
                style={{ resize: "vertical" }}
              />
            ...
            <ModalField label="Monetization">
              <textarea
                className="admin-input font-body"
                rows={3}
                placeholder="How do they make money?"
                value={form.context.monetization ?? ""}
                onChange={(e) =>
                  setForm((p) => ({
                    ...p,
                    context: { ...p.context, monetization: e.target.value },
                  }))
                }
                style={{ resize: "vertical" }}
              />
            ...
            <ModalField label="Content That Works">
              <textarea
                className="admin-input font-body"
                rows={3}
                placeholder="What content formats resonate?"
                value={form.context.content_that_works ?? ""}
                onChange={(e) =>
                  setForm((p) => ({
                    ...p,
                    context: {
                      ...p.context,
                      content_that_works: e.target.value,
                    },
                  }))
                }
                style={{ resize: "vertical" }}
            ...
            <ModalField label="Subtopics">
              <TagInput
                ...
                onAdd={() =>
                  addTag("subtopics", subtopicInput, setSubtopicInput)
                }
            ...
            <ModalField label="AI Opportunities">
              <textarea
                className="admin-input font-body"
                rows={3}
                placeholder="How can AI specifically help this niche?"
                value={form.context.ai_opportunities ?? ""}
                onChange={(e) =>
                  setForm((p) => ({
                    ...p,
                    context: { ...p.context, ai_opportunities: e.target.value },
                  }))
                }
                style={{ resize: "vertical" }}
              />
            ...
            <ModalField label="Seed Keywords">
              <TagInput
                ...
                onAdd={() =>
                  addTag("keywords_seed", keywordInput, setKeywordInput)
                }
            ...
            <ModalField label="Expert POV (used for 'From the trenches' callouts — only claims from this text are allowed)">
              <textarea
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, expert_pov: e.target.value }))
                }
                style={{ resize: "vertical" }}
              />
            </ModalField>
      ...
      <Dialog
        open={csvModalOpen}
        onOpenChange={(o) => !o && setCsvModalOpen(false)}
      >
        <DialogContent
          className="admin-card"
          style={{
