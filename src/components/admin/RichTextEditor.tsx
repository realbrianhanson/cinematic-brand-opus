...
const Divider = () => (
  <div
    style={{
      width: 1,
      height: 16,
      backgroundColor: "hsl(var(--admin-border))",
      margin: "0 4px",
    }}
  />
);
...
const MenuBar = ({
  ...
  return (
    ...
    >
      ...
      {linkOpen && (
        <div
          className="absolute left-0 right-0 flex items-center gap-2 z-50"
          style={{
            top: "100%",
            padding: "8px 12px",
            backgroundColor: "hsl(var(--admin-surface-2))",
        ...
        >
          <input
            ...
            autoFocus
            className="flex-1 h-7 rounded px-2 text-xs focus:outline-none"
            style={{
          ...
          {editor.isActive("link") && (
            <button
              ...
              style={{ color: "#e57373", border: "1px solid #e5737333" }}
            >
              Remove
            </button>
          )}
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        ...
        style:
          "font-size: 15px; line-height: 1.85; padding: 20px; color: hsl(var(--admin-text));",
      },
      handlePaste: (view, event) => {
        const text = event.clipboardData?.getData("text/plain")?.trim();
        if (
          text &&
          /^https?:\/\/.+\.(jpg|jpeg|png|gif|webp|svg|bmp|avif)(\?.*)?$/i.test(
            text,
          )
        ) {
  ...
  const handleVideoSelected = (html: string) => {
    ...
    if (html.startsWith("embed:")) {
      editor
        .chain()
        .focus()
        .insertContent({ type: "iframe", attrs: { src: html.slice(6) } })
        .run();
    } else if (html.startsWith("video:")) {
      editor
        .chain()
        .focus()
        .insertContent({ type: "video", attrs: { src: html.slice(6) } })
        .run();
    } else {
    ...
    <div className="admin-card" style={{ overflow: "hidden" }}>
      ...
      {editor && editor.isEditable && (
        <MenuBar
          ...
          onVideoEmbed={() => setShowVideoPicker(true)}
        />
      )}
