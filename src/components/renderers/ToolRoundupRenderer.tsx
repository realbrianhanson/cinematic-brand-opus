...
const ToolRoundupRenderer = ({
  ...
  const verdicts = useMemo(
    () => [
      ...new Set(
        tools.map((t) => t.verdict).filter((value): value is string => !!value),
      ),
    ],
    [tools],
  );
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-8">
        <div className="flex gap-2 flex-wrap">
          <FilterBtn
            ...
            onClick={() => {
              setVerdictFilter("");
              logEvent("filter_use", { filter: "verdict", value: "all" });
            }}
          >
            All
          </FilterBtn>
          {verdicts.map((v) => (
            <FilterBtn
              ...
              onClick={() => {
                setVerdictFilter(v);
                logEvent("filter_use", { filter: "verdict", value: v });
              }}
            >
              {v.replace(/-/g, " ")}
            </FilterBtn>
      ...
      {viewMode === "cards" ? (
        <div className="grid md:grid-cols-2 gap-5 mb-12">
          {filtered.map((tool, i) => {
            const vc =
              verdictColors[tool.verdict || "honorable-mention"] ||
              verdictColors["honorable-mention"];
            return (
              <div
                key={i}
