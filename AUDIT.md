...
### Brian-supplied PushTen artwork

Brian supplied a PushTen AI-Business Launch promotional image and authorized its use. Preserved the exact PNG source, optimized a complete 1600 × 900 WebP cover (162,204 bytes), and replaced the generated PushTen cover through the existing offer record. The image now appears on the Shop card, homepage preview, PushTen landing page, and social previews. The other offers and all non-cover product fields retain their previous values.

Source commit: `10e379d9b43605c409a60191650c2a24a7329a26`; Lovable deployment: `b5b76bc5-4daa-4879-bb1d-ed5aeef7c2db`. Verified the live deployment and image hash before applying the guarded, reviewed owner-content transaction. SQL verification covered idempotence, unrelated content preservation, and nine failure cases. Live browser checks passed on the three affected pages at 1440/390/320 widths, including image decoding, intact composition, metadata, and checkout destination; no page errors, production submissions, or tracking writes. [Release verification](https://github.com/realbrianhanson/cinematic-brand-opus/actions/runs/35489684473).

### Lynn Hutchison PushTen testimonial

Published Brian’s supplied testimonial verbatim, attributed to Lynn Hutchison, in a gold-accented quote block on the PushTen landing page. Added reusable plain-text quote formatting to the existing offer description, including multiple paragraphs and a separate author. The complete quote stays visible; Admin → Offers → Full description remains the editing path. No testimonial data is seeded into member installations.

Source commit `8b8cf69fd1f8bf316233a328aef9eb55ec41c1d1`; deployment `1966d3ed-6994-425c-a1f7-ea2c28ade782`; [full verification passed](https://github.com/realbrianhanson/cinematic-brand-opus/actions/runs/35490134197). Checked 51 targeted tests, build/typecheck/format/lint, code/TypeScript review, and guarded SQL success/idempotence/preservation plus ten failure cases. Published the renderer before applying the exact owner-content update. Production body matched the intended text, with all other non-timestamp fields unchanged.

Live checks at 1440/390/320 widths confirmed the exact full quote and name, readable layout, no overflow/page errors, server-rendered attribution, and preserved artwork/external checkout destination. Other offers stayed free of this testimonial. No test customer activity or tracking writes. See [offer testimonial documentation](docs/OFFER_TESTIMONIALS.md).

### Stronger homepage testimonial selection

Reviewed all 26 currently available source compilations against Brian's request for enthusiastic, specific, Brian-focused endorsements. Steve Cunningham now leads with his concrete app-building example, followed by E B Soloway and the complete lisa bond quote. Lynn Hutchison's clearly labeled, contiguous three-sentence excerpt, Marla Ray, and Patricia Pisterzi support them in the existing disclosure. All selected wording matches the supplied sources; no Francis references were removed and event feedback is not represented as PushTen product results. The full Lynn testimonial on the offer page is unchanged. Member presets receive none of these quotes.

Source commit `9b5d39c65786a5f06046e6dafacdd7b14bb3656c`; [GitHub verification 35490732982 passed](https://github.com/realbrianhanson/cinematic-brand-opus/actions/runs/35490732982). Deployment `d8628c87-d67e-4ff4-a25d-12c904507c77` was confirmed on the custom domain. Independent code and TypeScript reviews, targeted preset/render tests, typecheck, formatting, build, and full CI passed.

Read-only live browser verification at 1440/768/390/320 widths confirmed exact quote text, attribution/context, order, server-rendered names, three initially visible quotes, and all six after opening the disclosure. No overflow or browser errors occurred. The original hero video loaded normally after its intentional deferred activation. QA submitted no forms or production tracking writes. Screenshots and report: `/tmp/testimonial-curation-*-*.png` and `/tmp/testimonial-curation-live-qa-report.json`.

The separate [visual enrichment plan](docs/VISUAL_UPGRADE_PLAN.md) records page-specific recommendations and the verified current image-generation setup. This release does not generate new images or switch the automatic provider. Source curation and future editing guidance: [docs/TESTIMONIALS.md](docs/TESTIMONIALS.md).

### PushTen story artwork and cleaned outdoor portrait

Published reusable inline images in offer descriptions, with alt text, captions, uncropped rendering, accessible enlargement, and an admin **Insert image** helper. External offers repeat the same validated action after their description. Rewrote PushTen's copy around the cost of starting from scratch, its membership inclusions, Brian's playful AI Addict story, commercial rights, and a clear next step. Brian's two supplied promotional images appear in the relevant sections; the launch cover and Lynn Hutchison's full exact testimonial remain unchanged.

Removed the distant background walkers from Brian's supplied trail photograph with the built-in image-editing tool. Saved the source and edited master separately and placed an optimized version in his homepage story section. The image frame preserves the full portrait and puts its name caption below the image. Speaking's original portrait, the homepage video, and member identity defaults remain unchanged.

Runtime source `2be6bb7796616eafef15b4e8e5bb676691c8d049`; [GitHub verification 35491700212 passed](https://github.com/realbrianhanson/cinematic-brand-opus/actions/runs/35491700212). Lovable synced the exact source and published deployment `bfd0e770-92a9-4200-bc60-96519e9d81b1`, confirmed on the custom domain. All three new public WebP assets returned HTTP 200 with SHA-256 hashes matching the reviewed files before the guarded owner-content transaction was applied. Production readback matched the intended body, summary, and button text exactly; all other PushTen fields and both other offer rows retained their hashes.

Validation: 121 focused tests, typecheck, targeted lint, formatting, build, full CI, independent code/TypeScript and SQL reviews. The isolated content transaction passed idempotence, preservation, and 13 rejected guard cases. Twelve local image-modal cases covered desktop and short/narrow phones, including long captions, scrolling, visible close control, Escape, and focus return. Live checks at 1440/768/390/320 confirmed both inline images, exact testimonial, repeated external action, full portrait proportions, and no page errors or overflow. Independent screenshots found no visual blockers. Shop and other offer pages remained usable. QA created no customer submissions or tracking writes.

Details and editing guidance: [offer images](docs/OFFER_IMAGES.md), [portrait files and edit prompt](docs/BRIAN_TRAIL_PORTRAIT.md). Local evidence: `/tmp/pushten-story-live-qa-report.json`, `/tmp/pushten-story-publish-verification.json`, `/tmp/pushten-image-modal-qa-report.json`, and `/tmp/pushten-story-sql-test-report.json`. The existing 15 paused Lovable prompts were untouched; no paid Lovable coding prompt was used.
