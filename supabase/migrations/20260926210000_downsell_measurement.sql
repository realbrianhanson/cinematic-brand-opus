-- Extend existing optional measurement to the persisted downsell branch.
-- No historic observations, order facts or sessions are backfilled.
  ...
  FOR e IN SELECT value FROM jsonb_array_elements(_events) LOOP
    ...
    IF (e->>'type') LIKE 'upsell_%' AND (item.checkout_mode<>'native' OR NOT EXISTS(
      SELECT 1 FROM offers p WHERE p.id=(e->>'parent_offer_id')::uuid AND p.status='published' AND p.checkout_mode='native'
        AND (p.next_offer_id=item.id OR p.downsell_offer_id=item.id OR EXISTS (
          SELECT 1 FROM offer_orders prior WHERE prior.offer_id=p.id AND (prior.next_offer_id=item.id OR prior.downsell_offer_id=item.id) AND prior.status='fulfilled'
        ))
    )) THEN RETURN false; END IF;
  END LOOP;
...
BEGIN
  ...
  ELSE
    ...
    IF NOT EXISTS(
      ...
      WHERE parent.id=o.parent_order_id AND CASE WHEN parent.upsell_declined_at IS NOT NULL THEN parent.downsell_offer_id ELSE parent.next_offer_id END=o.offer_id
...
CREATE OR REPLACE FUNCTION public.admin_offer_journey_snapshot(_days integer DEFAULT 30)
...
BEGIN
  ...
  WITH cohort AS MATERIALIZED (
    ...
    FROM offer_orders o JOIN offer_orders parent ON parent.id=o.parent_order_id AND (parent.next_offer_id=o.offer_id OR parent.downsell_offer_id=o.offer_id)
