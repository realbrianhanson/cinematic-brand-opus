-- Automatic, bounded retries for private download-access email.
--
-- * Each attempt stores the provider HTTP status and a short plain-English
--   reason (no recipients, links, or payloads).
-- * Backoff is chosen by offers-api (5 min doubling to 6 h) and clamped here.
-- * A delivery that Resend definitely never accepted stops as 'failed' after
--   10 attempts. One that Resend MAY have accepted (timeout, 5xx, 409, missing
--   receipt, interrupted attempt) stops as 'needs_review' once it is 20 hours
--   past its first uncertain attempt, inside Resend's 24-hour idempotency
--   window, so a customer is never emailed twice.
-- * pg_cron calls offers-api every 15 minutes with the cron secret.
-- Safe to re-run. Sends nothing by itself.
...
-- Every 15 minutes. Unschedule with: SELECT cron.unschedule('offer-access-retry-15min');
DO $$
BEGIN
  IF to_regclass('cron.job') IS NULL THEN RETURN; END IF;
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'offer-access-retry-15min') THEN
    PERFORM cron.unschedule('offer-access-retry-15min');
  END IF;
  PERFORM cron.schedule(
    'offer-access-retry-15min',
    '*/15 * * * *',
    $cmd$
