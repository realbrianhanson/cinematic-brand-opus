...
export async function remediatePostFacts(
  ...
  if (fc.remediated === true)
    return { ok: false, reason: "already remediated" };

  const contradicted = fc.claims.filter(
    (c: any) => c.verdict === "contradicted",
  );
  ...
  const factInstructions = [
    contradicted.length > 0
      ? `CONTRADICTED CLAIMS — DELETE these entirely (sentence + supporting context):\n${contradicted
          .map((c: any, i: number) => `${i + 1}. "${c.claim}"`)
          .join("\n")}`
      : "",
    unverified.length > 0
...
- Do not touch any claim marked verified.`;
  ...
  if (revised.error)
    return { ok: false, reason: `revise failed: ${revised.error}` };
  ...
  try {
    const { data: cta } = await supabase
      ...
      .maybeSingle();
    if (merged.content)
      merged.content = linkifyEventMentions(merged.content, cta?.cta_url);
    if (merged.excerpt)
      merged.excerpt = linkifyEventMentions(merged.excerpt, cta?.cta_url, {
        maxLinks: 1,
      });
  } catch {
    /* non-fatal */
